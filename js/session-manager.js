...
// ============ FIXED: USE CORS PROXY ============
const SESSION_ORIGINAL_URL = "https://script.google.com/macros/s/AKfycbxdAGmjmDECdLxCsXCvnnnLktsGXLpUKrrM7t_ZZ9aEwiJ_mB0k-CVF57khbAqU2hlX2w/exec";
const SESSION_PROXY_URL = "https://corsproxy.io/?" + encodeURIComponent(SESSION_ORIGINAL_URL);
...
class SessionManager {
    constructor() {
        ...
        // USE PROXY URL
        this.API_URL = window.API_URL || SESSION_PROXY_URL;
        console.log("Session Manager: Using API URL:", this.API_URL);
    }
    ...
    // Validate session with server - FIXED to use proxy
    async validateSession() {
        ...
        try {
            // USE PROXY URL - NO REDECLARATION
            ...
            const response = await fetch(this.API_URL, {  // ← USE this.API_URL
                method: 'POST',
                body: formData,
    ...
    // Update user info from server - FIXED to use proxy
    async updateUserInfo(username) {
        try {
            // USE PROXY URL
            const response = await fetch(`${this.API_URL}?action=users&t=${Date.now()}`);
            const data = await response.json();
    ...
    // Force guest restrictions - ONLY SHOW DASHBOARD
        ...
        if (isGuest) {
            ...
            document.querySelectorAll('.nav-item, .mobile-nav-item, .sidebar li, .sidebar a, .main-nav a, [data-page]').forEach(el => {
                ...
                if (!isDashboard) {
                    ...
                    if (el.closest('.mobile-nav-item')) el.closest('.mobile-nav-item').style.display = 'none';
                }
            });
            ...
            document.querySelectorAll('a[href*="dashboard"], [data-page="dashboard"]').forEach(el => {
                ...
                if (el.closest('.mobile-nav-item')) el.closest('.mobile-nav-item').style.display = 'flex';
                if (el.closest('.nav-item')) el.closest('.nav-item').style.display = 'flex';
            });
